...
export function useTranslation(ns: string | Array<string>) {
	const lng = useLocale();

	const translator = useTransAlias(ns);
	const { i18n } = translator;
	useCustomTranslationImplem(i18n, lng, runsOnServerSide);

	return translator;
}

function useCustomTranslationImplem(i18n: i18n, lng: Locales, runsOnServerSide: boolean) {
